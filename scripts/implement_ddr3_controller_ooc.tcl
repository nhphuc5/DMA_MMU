# Out-of-context implementation for a realistic placed/routed Fmax and
# resource check of the project-owned AXI DDR3 controller (digital portion).
...
if {[info exists ::env(DDR3_CONTROLLER_IMPL_DIR)]
        && $::env(DDR3_CONTROLLER_IMPL_DIR) ne ""} {
    set out_dir [file normalize $::env(DDR3_CONTROLLER_IMPL_DIR)]
} else {
    set out_dir [file join $root_dir "build/vivado/ddr3_controller_impl"]
}
...
create_project -force DDR3_Controller_Impl $out_dir -part $part_name
...
create_clock -name axi_clk -period $period [get_ports aclk]
...
report_utilization -hierarchical -file \
    [file join $report_dir "utilization_routed.rpt"]
report_timing_summary -delay_type max -max_paths 20 \
    -report_unconstrained -check_timing_verbose \
    -file [file join $report_dir "timing_routed.rpt"]
write_checkpoint -force [file join $out_dir "axi_ddr3_controller_routed.dcp"]
...
set fd [open [file join $report_dir "implementation_summary.txt"] w]
puts $fd "CUSTOM AXI DDR3 CONTROLLER OOC IMPLEMENTATION"
puts $fd "Part: $part_name"
puts $fd "Constraint: $period ns"
puts $fd [format "Worst routed slack: %.3f ns" $slack]
puts $fd [format "Estimated routed Fmax: %.3f MHz" $fmax]
puts $fd "DSP cells: $dsp_count"
close $fd
if {$dsp_count != 0} {
    error "Zero-DSP requirement failed: found $dsp_count DSP cells"
}
puts [format "DDR3 controller implementation: DSP=%d, routed Fmax=%.3f MHz" \
    $dsp_count $fmax]
close_project
